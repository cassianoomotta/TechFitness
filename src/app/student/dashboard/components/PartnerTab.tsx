...
interface ExerciseComparisonItem {
  exerciseId: string;
  exerciseName: string;
  muscleGroup: string;
  myMax: number;
...
interface ComparisonData {
  myInfo: ComparisonInfo;
  partnerInfo: ComparisonInfo;
  sharedExercises: SharedExercise[];
  exerciseComparison: ExerciseComparisonItem[];
}

interface PartnerTabProps {
  partnerSearchQuery: string;
  setPartnerSearchQuery: (value: string) => void;
  partners: Partner[];
        ...
        <div className="glass-card rounded-2xl p-6 border border-[#E2E8F0]/80">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider block">Buscar Parceiro (Nome ou E-mail)</label>
              <input
                type="text"
                placeholder="Digite o nome ou e-mail..."
                value={partnerSearchQuery}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPartnerSearchQuery(e.target.value)}
                className="w-full p-3 rounded-xl bg-white border border-[#E2E8F0] focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 outline-none text-xs text-[#0F172A] transition-all"
              />
            </div>

            <div className="space-y-1">
              <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider block">Escolha seu Parceiro de Treino</label>
              {partners.length === 0 ? (
                ...
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleSelectPartner(e.target.value)}
                  className="w-full p-3 rounded-xl bg-white border border-[#E2E8F0] focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 outline-none text-xs text-[#0F172A] transition-all"
                >
        ...
        {comparisonLoading ? (
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[1, 2].map((i: number) => (
                <div key={i} className="h-36 skeleton-shimmer rounded-2xl"></div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6 border border-[#E2E8F0]/80">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-[11px] bg-blue-50 border border-blue-200 px-2 py-0.5 rounded font-bold text-[#2563EB]">
              ...
              {comparison.sharedExercises.length === 0 ? (
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {(() => {
                    const groups = comparison.sharedExercises.reduce((acc: Record<string, SharedExercise[]>, ex: SharedExercise) => {
                      const group = ex.muscleGroup || "Outros";
                      if (!acc[group]) acc[group] = [];
                      acc[group].push(ex);
                      return acc;
                    }, {} as Record<string, SharedExercise[]>);
                    
                    return Object.entries(groups).map(([group, exercises]) => (
                      <div key={group}>
                        <p className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1.5 mt-3 first:mt-0">
                          {group}
                        </p>
                        <div className="space-y-1.5">
                          {(exercises as SharedExercise[]).map((ex: SharedExercise) => (
                            <div key={ex.exerciseId} className="p-2.5 bg-zinc-50 rounded-lg border border-[#E2E8F0]/60 flex items-center justify-between gap-3 hover:border-blue-200 hover:shadow-sm transition-all duration-300">
                              <div className="min-w-0">
                                <p className="text-xs font-semibold text-[#0F172A] truncate">{ex.name}</p>
                                <p className="text-[10px] text-[#94A3B8]">{ex.equipment}</p>
                              </div>
                              <span className="text-[10px] bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded font-bold flex-shrink-0 border border-emerald-200/50">
                                Em Comum ✓
                              </span>
                            </div>
                          ))}
            ...
            {comparison.exerciseComparison.length > 0 && (
              <div className="glass-card rounded-2xl p-6 border border-[#E2E8F0]/80">
                ...
                {comparison.exerciseComparison.filter((ex: ExerciseComparisonItem) => ex.myMax > 0 || ex.partnerMax > 0).length === 0 ? (
                  <div className="p-6 text-center border border-dashed border-[#E2E8F0] rounded-xl">
                  ...
                  <div className="space-y-4">
                    {comparison.exerciseComparison
                      .filter((ex: ExerciseComparisonItem) => ex.myMax > 0 || ex.partnerMax > 0)
                      .map((ex: ExerciseComparisonItem) => {
                        ...
                        return (
                          <div key={ex.exerciseId} className="p-4 rounded-xl bg-zinc-50 border border-[#E2E8F0]/60 hover:shadow-sm transition-all duration-300">
                            <div className="flex items-center justify-between mb-3">
                              <div>
                                <h5 className="text-xs font-bold text-[#0F172A]">{ex.exerciseName}</h5>
                                <p className="text-[10px] text-[#94A3B8]">{ex.muscleGroup}</p>
                              </div>
                              ...
                              {isTied && (
                                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200/50">
                                  Empate ⚔️
                                </span>
                              )}
                            </div>
