...
import { Dumbbell, User, Lock, Mail, UserPlus, Loader2, ArrowRight, Shield } from "lucide-react";
  ...
  return (
    <main className="flex-1 flex items-center justify-center p-4 md:p-8 bg-slate-950 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg glass-card rounded-2xl p-6 md:p-10 shadow-2xl relative z-10 animate-fade-in transition-all">
        ...
        {apiError && (
          <div className="mb-6 p-4 rounded-lg bg-red-950/50 border border-red-500/30 text-red-200 text-sm text-center">
            {apiError}
          </div>
        )}

        {success && (
          <div className="mb-6 p-4 rounded-lg bg-emerald-950/50 border border-emerald-500/30 text-emerald-200 text-sm text-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className={`flex flex-col items-center gap-3 p-4 rounded-xl border transition-all ${
                  role === "STUDENT"
                    ? "bg-slate-900/80 border-emerald-500 text-emerald-400 shadow-md shadow-emerald-500/10"
                    : "bg-slate-900/30 border-zinc-800/80 text-zinc-400 hover:border-zinc-700 hover:bg-slate-900/50"
                }`}
              >
                <User className="w-6 h-6" />
                <div className="text-center">
                  <p className="text-sm font-semibold text-white">Aluno</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Quero treinar</p>
                </div>
              </button>
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">Quero prescrever</p>
                </div>
              </button>
            </div>
            {errors.role && (
              <p className="text-xs text-red-400 mt-1">{errors.role[0]}</p>
            )}
          </div>
