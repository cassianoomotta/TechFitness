    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col text-zinc-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="p-2.5 rounded-xl border border-zinc-800 hover:border-red-500/30 hover:bg-red-550/5 text-zinc-500 hover:text-red-650 transition-all cursor-pointer"
            >
      ...
      <section className="bg-zinc-900/60 border-b border-zinc-800 py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-3">
          <Link
            href="/trainer/dashboard"
            className="p-2 rounded-lg border border-zinc-800 hover:border-zinc-300 bg-zinc-900 text-zinc-550 hover:text-zinc-100 transition-all"
          >
      ...
      {loading ? (
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8">
          ...
          <section className="w-full lg:w-5/12">
            <div className="glass-card rounded-2xl p-6 border border-zinc-800 bg-zinc-900 shadow-sm">
              <h3 className="font-display font-semibold text-sm text-zinc-100 uppercase tracking-wider mb-5 flex items-center gap-2">
                <Plus className="w-4 h-4 text-cyan-500" /> Nova Medição
              </h3>
              ...
              {success && (
                <div className="mb-4 p-3 rounded-lg bg-cyan-950/20 border border-cyan-500/30 text-cyan-300 text-xs text-center font-semibold animate-pulse">
              ...
              <form onSubmit={handleSaveMeasurement} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="Ex: 78.5"
                      className="w-full px-3.5 py-2 rounded-xl bg-zinc-900 border border-zinc-800 focus:border-cyan-500 outline-none text-xs text-zinc-200 placeholder-zinc-450 transition-all font-mono"
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="pt-4 border-t border-zinc-900 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div className="flex flex-col items-center">
                      <label className="w-full aspect-square rounded-xl bg-zinc-50 border border-zinc-800 hover:border-zinc-350 transition-all flex flex-col items-center justify-center cursor-pointer relative overflow-hidden">
                        {photoFrente ? (
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-cyan-500 hover:bg-cyan-600 text-white font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:pointer-events-none mt-4"
                >
                  {actionLoading ? (
          ...
          <section className="w-full lg:w-7/12 space-y-6">
            <h3 className="text-xs font-bold text-zinc-550 uppercase tracking-widest flex items-center gap-2">
              <Calendar className="w-4 h-4 text-cyan-500" /> Histórico de Medidas ({history.length})
            </h3>

            {history.length === 0 ? (
              <div className="glass-card rounded-2xl p-12 text-center text-zinc-500 border border-zinc-800 bg-zinc-900 shadow-sm">
                <Activity className="w-10 h-10 mx-auto text-zinc-300 mb-3" />
                <p className="text-xs font-semibold text-zinc-200">Nenhuma medição registrada</p>
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
                {history.map((m) => (
                  <div
                    key={m.id}
                    className="glass-card rounded-2xl p-5 border border-zinc-800 bg-zinc-900 space-y-4 shadow-sm hover:border-cyan-500/30 transition-all duration-300"
                  >
                    {/* Header do Card */}
                    <div className="flex justify-between items-center pb-3 border-b border-zinc-900">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-cyan-500" />
                        <h4 className="text-xs font-bold text-zinc-100">{formatDate(m.date)}</h4>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs text-zinc-650">
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">BF (Gordura)</span>
                        <span className="text-zinc-100 font-mono font-bold">{m.bodyFat !== null ? `${m.bodyFat}%` : "--"}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">Peitoral</span>
                        <span className="text-zinc-100 font-mono font-bold">{m.chest !== null ? `${m.chest}cm` : "--"}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">Cintura</span>
                        <span className="text-zinc-100 font-mono font-bold">{m.waist !== null ? `${m.waist}cm` : "--"}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">Braços (E/D)</span>
                      ...
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">Coxas (E/D)</span>
                        <span className="text-zinc-100 font-mono font-bold">
                      ...
                      <div>
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold">Panturrilhas (E/D)</span>
                        <span className="text-zinc-100 font-mono font-bold">
                    ...
                    {m.photos && m.photos.length > 0 && (
                      <div className="pt-3 border-t border-zinc-900">
                        <span className="text-[9px] text-zinc-500 uppercase tracking-wider block font-semibold mb-2 font-display">Fotos Comparativas</span>
                        <div className="flex gap-2">
                          {m.photos.map((photo, pIdx) => (
                            <div
                              key={pIdx}
                              onClick={() => setSelectedPhotoForZoom(photo)}
                              className="w-14 h-14 rounded-lg bg-zinc-50 border border-zinc-800 overflow-hidden relative cursor-zoom-in hover:border-cyan-500 transition-all"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="relative max-w-sm max-h-[80vh] w-full flex items-center justify-center">
            <button
              ...
              className="absolute -top-12 right-0 p-2 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-white transition-all cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            <img
